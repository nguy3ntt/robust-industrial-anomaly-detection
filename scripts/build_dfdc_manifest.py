"""Build and audit a leakage-safe DFDC Preview video manifest."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=Path("configs/data/dfdc-preview.yaml"),
...
def apply_overrides(config: DfdcConfig, args: argparse.Namespace) -> DfdcConfig:
    updates: dict[str, object] = {}
    if args.dataset_root:
        dataset_root = args.dataset_root.resolve()
        updates["dataset_root"] = dataset_root
        updates["metadata_path"] = dataset_root / "dataset.json"
    if args.output_dir:
...
def main() -> int:
    ...
    except (DfdcConfigurationError, OSError, ValueError) as exc:
